...
1. **Does DuckDB Python (current release, 1.5.x) consume a PyCapsule-protocol object natively via its relation/replacement-scan mechanism, or does it still require pyarrow as an intermediary?**
...
2. **Should the strict-mode/`copy_report()` exception hierarchy and report shape (Code Examples) be locked now or left fully to implementation-time discretion?**
