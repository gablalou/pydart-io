...
### PyPI Distribution Name Resolution (post-research correction)
- **D-41:** Phase 4 research (2026-07-27) found that `pydart` is already registered on real PyPI by an unrelated, dormant project ("Python Interface for DART Simulator," github.com/sehoonha/pydart, last released 2016) — confirmed via direct PyPI registry query (`https://pypi.org/pypi/pydart/json` returned HTTP 200 with that project's metadata). This conflicts with D-32/D-33 as originally written (both assumed "pydart" was claimable as the literal PyPI listing name). **User-confirmed resolution:** publish to PyPI under the distribution name **`pydart-io`** (confirmed available via direct registry check, same method). `import pydart`, the Rust crate names (`pydart-core`, `pydart-python`), the compiled extension (`pydart._pydart`), and the exception (`pydart.PydartError`) are all unchanged — only `pyproject.toml`'s `[project].name` field and the PyPI listing itself use `pydart-io`. This supersedes D-32/D-33's implicit assumption that the PyPI listing name is literally "pydart"; D-32's "pip install pydart must work for real users" is now satisfied as `pip install pydart-io` (which installs the `pydart` importable package, i.e. `import pydart` after installing `pydart-io`).
