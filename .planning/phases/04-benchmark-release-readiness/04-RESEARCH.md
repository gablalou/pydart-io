...
**Finding surfaced and resolved during this research session:** `pydart` is **already registered on PyPI** — it belongs to an unrelated project ("Python Interface for DART Simulator" by Sehoon Ha, `github.com/sehoonha/pydart`) `[VERIFIED: PyPI registry, https://pypi.org/pypi/pydart/json, HTTP 200 returned 2026-07-27]`. This conflicted with D-32/D-33 as originally written. **Resolved via D-41 (04-CONTEXT.md): publish under distribution name `pydart-io`** (confirmed available); `import pydart` and all Rust crate/extension/exception names are unchanged. The planner should use `pydart-io` directly as the publish target — see Pitfall 6 (RESOLVED) and Open Questions.
...
### Pitfall 6 (RESOLVED — see D-41 in 04-CONTEXT.md): `pydart` is not available on the real PyPI index
...
**Resolution (D-41, user-confirmed 2026-07-27):** Publish under distribution name `pydart-io` (confirmed available on real PyPI). `import pydart` and all Rust crate/extension/exception names are unchanged — only `pyproject.toml`'s `[project].name` and the publish workflow's target name change. Planner: use `pydart-io` directly; this is no longer an open decision.
**Warning signs:** A publish workflow that assumes `pydart` (not `pydart-io`) will succeed on first `twine upload`/`gh-action-pypi-publish` run — it will fail with a 403/name-already-claimed error.
...
1. ~~**How should the D-32/D-33 vs. real-PyPI-name-collision conflict be resolved?**~~ — **RESOLVED** (user decision, 2026-07-27, recorded as **D-41** in 04-CONTEXT.md): publish under PyPI distribution name **`pydart-io`** (confirmed available). `import pydart` and all Rust crate/extension/exception names are unchanged. Planner: use `pydart-io` as the `pyproject.toml` `[project].name` and the PyPI/publish-workflow target; do not re-raise this question.
