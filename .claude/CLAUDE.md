...
| pandas >= 3.0 | Arrow PyCapsule Interface (export-only) | Floor raised from `>=2.2` to `>=3.0` (D-36): Copy-on-Write (CoW) is unconditional only from pandas 3.0 onward, and the zero-copy numpy buffer borrow's post-borrow-mutation safety (WR-02) relies entirely on CoW being active — pre-3.0, CoW is off by default and the borrow has no independent immutability guarantee. pandas only *exports* via `__arrow_c_*__` (no import support as of current pandas 3.0.x) — round-trip *into* pandas still goes through `pyarrow`-style `to_pandas()`/`ArrowDtype` construction, not the capsule protocol in reverse. Confirm current pandas import-side support before assuming full bidirectional capsule interop. |
