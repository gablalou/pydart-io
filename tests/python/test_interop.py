...
(CAP-02, via `pydart.from_arrow`) is proven from pyarrow and Polars, and the two documented
...
1.5.4 at module-import time: does `duckdb.sql("FROM <pydart.Table instance>")` consume a pydart
`Table` natively via DuckDB's own replacement-scan mechanism, with no pyarrow intermediary?

**Result: YES.** `duckdb.sql("FROM pydart_table").arrow().read_all()` (where `pydart_table` is a
real `pydart.Table` local variable, no `pyarrow`/`register()` call involved) returns a correct
...
`pydart.from_arrow`'s own import path using an instrumented wrapper object.
"""
...
def _probe_duckdb_native_consumption() -> bool:
    ...
    consume a pydart `Table` natively via `duckdb.sql("FROM <obj>")`, without pyarrow as an
    ...
    probe_table = pydart.Table.from_pandas(_numeric_frame())
    try:
...
def _duckdb_export_round_trip(pydart_table: "pydart.Table") -> pa.Table:
    ...
    if DUCKDB_NATIVE_CONSUMPTION:
        relation = duckdb.sql("FROM pydart_table")
    ...
    pa_table = pa.table(pydart_table)
...
def test_pyarrow_accepts_pydart_table_export():
    df = _numeric_frame()
    pydart_table = pydart.Table.from_pandas(df)
...
def test_polars_accepts_pydart_table_export():
    df = _numeric_frame()
    pydart_table = pydart.Table.from_pandas(df)

    pl_df = pl.from_arrow(pydart_table)

    assert pl_df.shape == (len(df), len(df.columns))
    assert pl_df.columns == list(df.columns)


def test_duckdb_accepts_pydart_table_export():
    df = _numeric_frame()
    pydart_table = pydart.Table.from_pandas(df)

    pa_result = _duckdb_export_round_trip(pydart_table)

    assert pa_result.num_rows == len(df)
    assert pa_result.schema.names == list(df.columns)


# --- CAP-02: import from pyarrow Table and Polars DataFrame via pydart.from_arrow --------------


def test_from_arrow_imports_pyarrow_table():
    pa_table = pa.table({"a": [1, 2, 3], "b": [1.5, 2.5, 3.5]})

    imported = pydart.from_arrow(pa_table)
...
def test_from_arrow_imports_polars_dataframe():
    ...
    imported = pydart.from_arrow(pl_df)
    result = imported.to_pandas()
...
def test_from_arrow_rejects_broken_stream_dunder_without_panicking():
    ...
    invoked must surface a clean `pydart.PydartError`, never a panic/segfault (T-01-08)."""
    ...
    with pytest.raises(pydart.PydartError):
        pydart.from_arrow(BrokenStream())
...
class _CountingArrowStreamWrapper:
    ...
    `__arrow_c_stream__` is invoked -- proves `pydart.from_arrow` consumes a foreign object's
...
def test_from_arrow_consumes_foreign_stream_dunder_exactly_once():
    ...
    imported = pydart.from_arrow(wrapper)
