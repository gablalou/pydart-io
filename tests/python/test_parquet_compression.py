...
string raises `pydart.PydartError` naming the offending string, never silently substituting snappy.

Row-group counts are read via `pyarrow.parquet.ParquetFile(path).num_row_groups` since Pydart does
...
def test_unknown_codec_raises_pydart_error(tmp_path, bad_codec):
    """D-29 rejection: an unsupported codec string raises pydart.PydartError naming the offending
...
def test_row_group_size_boundary(tmp_path):
    ...
    table_m = pydart.Table.from_pandas(df_m)
    ...
    table_m_plus_1 = pydart.Table.from_pandas(df_m_plus_1)
...
def test_empty_and_single_row_group_counts(tmp_path):
    ...
    result = pydart.Table.from_parquet(str(empty_path))
    assert len(result.to_pandas()) == 0

    single_df = _numeric_frame(1)
    single_table = pydart.Table.from_pandas(single_df)
